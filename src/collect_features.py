...
def collect_regional(cfg: DictConfig) -> tuple(
    ...
    filenames = [file for file in os.listdir(cfg.paths.features_region) if 'Раздел' in file]
        ...
        if len(features)!=0:
            ...
            for f in features:
                ...
                for row in data.itertuples():
                    if not any(reg_name in row[1] for reg_name in regions['Name'].tolist()):
                ...
                data[data_cols] = data[data_cols].replace(to_replace = ["-", ".*"], value = 0, regex=True)
...
def apply_regional(row: pd.Series,
                   region_features:list,
                   tags_order: list,
                   codes: list) -> pd.Series:
    ...
    # For any other region go on
        ...
        array_extract = region_features[region_index,
                        reg_year,
                        :]
...
def collect_extra_features(cfg: DictConfig) -> pd.DataFrame:
    ...
    folders = [folder for folder in os.listdir(cfg.paths.features_extra) if 'zip' not in folder]
    ...
    for folder in folders:
        ...
        for filename in tqdm(os.listdir(path_folder)):
            ...
            if filename.endswith(".xml"):
                ...
                tree = ET.parse(os.path.join(cfg.paths.features_extra, folder, filename))
                root = tree.getroot()
                ...
                for child in root[1:]:                     
                    ...
                    for subchild in child[:1]:
                        row = {}
                        inn = list(map(subchild.attrib.get, filter(lambda x:x in ['ИННЮЛ', 'ИННФЛ'], subchild.attrib)))
                    
                        row['ИНН'] = inn[0]
                        row['КатСубМСП'] = child.attrib['КатСубМСП']
                        row['ССЧР'] = child.get('ССЧР') # puts None if key doesn't exist
                    xml_data.append(row)
    ...
    extra_feat = extra_feat.drop_duplicates(subset=['ИНН'], keep='first',)
    bad_index = extra_feat[extra_feat['ИНН']=='~~~~~~~~~~~~']
    if not bad_index.empty:
        extra_feat = extra_feat.drop(index = [bad_index.index[0]])
    extra_feat = extra_feat.astype({'ИНН': 'int64', 'ССЧР': 'int8', 'КатСубМСП': 'int8'})
...
def add_features(cfg: DictConfig) -> None:
    ...
    extra_features = open_pickle(cfg.paths.pkls, cfg.files.extra_features)

    print('Stacking all features...')
    df = open_pickle(cfg.paths.pkls, cfg.files.companies)
    df = df.progress_apply(lambda row: apply_regional(row, region_features, tags_order, codes), axis=1)

    # Extra features
    result = df.merge(extra_features, on = 'ИНН', how = 'left')

    # Save
    save_pickle(cfg.paths.pkls, cfg.files.companies_feat, result)
