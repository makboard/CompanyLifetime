...
def load_egrul(cfg: DictConfig) -> pd.DataFrame:
    ...
    df["reg_date"] = pd.to_datetime(df["reg_date"], dayfirst=False)
    df["end_date"] = pd.to_datetime(df["end_date"], dayfirst=False)
    df["min_num"] = pd.to_datetime(df["min_num"], dayfirst=False)
    df["max_num"] = pd.to_datetime(df["max_num"], dayfirst=False)
    ...
    for filename in tqdm(os.listdir(cfg.paths.msp)):
        if filename.endswith(".xlsx"):
            ...
            with warnings.catch_warnings(record=True):
                ...
                df = pd.read_excel(
                    os.path.join(cfg.paths.msp, filename), header=0, skiprows=2
                )
