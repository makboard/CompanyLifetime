...
def get_date(reg_date: pd.Timestamp,
             ogrn: int,
             msp_date: pd.Timestamp) -> pd.Timestamp:
...
def get_activity(df: pd.DataFrame,
                 INN: int) -> str:
    '''Fills empty 'Основной вид деятельности' cell from duplicating row of the same company'''
    df_mini = df[df['ИНН']==INN].sort_values(by=['Основной вид деятельности'], ascending=True)
...
def merge_data(cfg: DictConfig) -> None:
    ...
    df_msp['Дата исключения из реестра'] = pd.to_datetime(df_msp['Дата исключения из реестра'], dayfirst=True)

    # Merge two datasets by OGRN number
    merged_ogrn = pd.merge(left=df_msp, right=df_egrul, how='left', left_on='ОГРН', right_on='ogrn')
    print('merged')
    merged_ogrn['reg_date'] = merged_ogrn.progress_apply(lambda row: get_date(row['reg_date'], row['ОГРН'], row['Дата включения в реестр']), axis=1)
    merged_ogrn['end_date'] = merged_ogrn.progress_apply(lambda row: row["Дата исключения из реестра"] if pd.isnull(row['end_date']) else row["end_date"], axis=1)
    
    # Reduce unique values in "Основной вид деятельности" by lowering detailes proveded. For example, code 58.11.1 --> 58
    merged_ogrn['Основной вид деятельности'] = list(map(lambda item: item[:2], map(str, merged_ogrn['Основной вид деятельности'])))
    # Fill 'na' in "Основной вид деятельности"
    merged_ogrn['Основной вид деятельности'] = merged_ogrn.progress_apply(lambda row: get_activity(merged_ogrn, row['ИНН']) if row['Основной вид деятельности']=='na' else row['Основной вид деятельности'], axis=1)

    # Calculate lifetime of a company as a separate column
    merged_ogrn['lifetime'] = (merged_ogrn['end_date'].dt.year - merged_ogrn['reg_date'].dt.year)*12 + (merged_ogrn['end_date'].dt.month - merged_ogrn['reg_date'].dt.month)
    # merged_ogrn['lifetime'] = (merged_ogrn['end_date'] - merged_ogrn['reg_date'] ).astype('timedelta64[M]').astype('Int64')   
    
    # Drop duplicates
    merged_ogrn = merged_ogrn.sort_values("reg_date", ascending = False).drop_duplicates(subset=['ОГРН'], keep='first')
    
    companies_closed = merged_ogrn[merged_ogrn['Дата исключения из реестра'].notna()]
    
    # Drop few columns
    companies_closed.drop(columns=['ogrn', 'opf_id', 'full_name', 'okved_id', 'inn', 'capital',
