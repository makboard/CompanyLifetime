...
def metrics_print(y_train: pd.DataFrame,
    ...
    metrics_dict[key]['train']['r2'], metrics_dict[key]['train']['mae'], metrics_dict[key]['train']['rmse'] = r2_train, mae_train, rmse_train
    metrics_dict[key]['test']['r2'], metrics_dict[key]['test']['mae'], metrics_dict[key]['test']['rmse'] = r2_test, mae_test, rmse_test

    return metrics_dict


def linear_regression(cfg: DictConfig,
    ...
    key = 'lr'
    metrics_dict = metrics_print(y_train,
                                 train_predictions,
                                 y_test,
                                 test_predictions,
                                 metrics_dict,
...
def ridge_regression(cfg: DictConfig,
                      X_train: pd.DataFrame,
                      y_train: pd.DataFrame,
                      X_test: pd.DataFrame,
                      y_test: pd.DataFrame,
...
def xgb_regression(cfg: DictConfig,
                    X_train: pd.DataFrame,
                    y_train: pd.DataFrame,
                    X_test: pd.DataFrame,
                    y_test: pd.DataFrame,
