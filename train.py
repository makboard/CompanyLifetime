...
def make_indices(X: pd.DataFrame) -> tuple[list, list, list]:
    ...
    categorical_indices = np.array([(column in categorical_columns) for column in X.columns], dtype = bool)
...
def preprocess_X(X: pd.DataFrame) -> pd.DataFrame:
    ...
    X_cat = pd.DataFrame(
        X_cat.toarray().astype(np.int8), 
        columns=enc.get_feature_names_out()
    )

    # Concat all columns
    X = pd.concat([X[binary_columns].reset_index(drop=True), X_num.reset_index(drop=True), X_cat.reset_index(drop=True)], axis = 1)

    return X


def train_test(cfg: DictConfig) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    '''Perform train/test split'''
    print('Collecting data...')
    data = open_pickle(cfg.paths.pkls, cfg.files.companies_feat)
    ...
    X['Основной вид деятельности'] = X.apply(lambda row: row["Основной вид деятельности"][1:2] if row["Основной вид деятельности"][0]=='0' else row["Основной вид деятельности"][:2], axis=1)
    X['Регион'] = X.apply(lambda row: row["Регион"][1:2] if row["Регион"][0]=='0' else row["Регион"][:2], axis=1)

    # Convert columns data types
    X['Тип субъекта'] = (X['Тип субъекта'] == 'Индивидуальный предприниматель').astype(int)
    X[['Основной вид деятельности','Регион']] = X[['Основной вид деятельности', 'Регион']].astype(int)
...
@hydra.main(version_base=None, config_path=os.path.join(os.getcwd(),"config"), config_name="regression_config.yaml")
def run_train(cfg: DictConfig):
    metrics_dict = {}
    X_train, X_test, y_train, y_test = train_test(cfg)
    metrics_dict = linear_regression(cfg, X_train, y_train, X_test, y_test, metrics_dict)
